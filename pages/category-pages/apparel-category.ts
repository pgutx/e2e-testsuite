...
export class CategoryApparel{
    ...
    constructor(page:Page){
      ...
      this.tshirtsLink = page.getByRole('link', { name: 'T-shirts', exact: true });
      ...
      this.itemName = page.getByTitle('Fiorella Purple Peep Toes');
      this.itemImage = page.locator('div.col-md-3:nth-child(8) > div:nth-child(2) > a:nth-child(1) > img:nth-child(1)');
      this.itemViewButton = page.getByRole('link', { name: 'View', exact: true });
      this.itemReviewButton = page.getByRole('link', { name: 'Write Review' });
      this.itemPrice = page.getByText('$110.00').first();
      this.itemCartButton = page.getByRole('link', { name: '' }).nth(3);
    }
