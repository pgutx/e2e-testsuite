...
export class CategoryBooks{
    ...
    readonly paperbackLink: Locator;
    ...
    constructor(page:Page){
      this.page = page;
      this.audioLink = page.getByRole('link', { name: 'Audio CD', exact: true });
      this.paperbackLink = page.getByRole('link', { name: 'Paperback', exact: true });
      ...
      this.itemName = page.getByTitle('Paper Towns by John Green');
      this.itemImage = page.locator('div.thumbnails:nth-child(4) > div:nth-child(1) > div:nth-child(2) > a:nth-child(1) > img:nth-child(1)');
      this.itemViewButton = page.getByRole('link', { name: 'View', exact: true });
      this.itemReviewButton = page.getByRole('link', { name: 'Write Review' });
      this.itemPrice = page.getByText('$7.99').first();
      this.itemCartButton = page.getByRole('link', { name: '' }).first();
    }

    async clickAudioLink(){
      await this.audioLink.click();
    }

    async clickPaperbackLink(){
      await this.paperbackLink.click();
    }
